---
name: core-values-charter
category: charter
description: Maps Atlas branding values and team charter standards to runtime checks operators and roles can verify.
audience: [all]
tags: [charter, values, governance, runtime-checks]
---

# Core values and charter runtime checks

Maps Atlas branding values and team charter standards to what operators and roles can verify at runtime. Full charter text lives in `rules/team-charter.md`. Values list in `branding-values.md`.
...
| Built to scale | Lazy context, workflow presets | `atlas-core.md` stays small; playbooks load on demand; workflow picks phase set |

## Charter standards → runtime checks

| Standard | Runtime signal |
| --- | --- |
| Token efficiency | Estimation huddle before first specialist delegation; `Predicted` and role estimates in `budget.md`; pipeline validator warns on kickoff gaps |
...
| Orchestration only (lead) | Specialist artifacts appear after delegation; lead edits `team.json`, `budget.md`, gate sequencing only |
...
2. Estimation huddle via delegation: each role returns light | medium | heavy.
3. atlas-ai-eng aggregates; lead writes `budget.md` from `budget-template.md`.
4. Heavy predicted usage needs token-budget gate approval before build delegations start.
...
- Kickoff gaps (missing estimates, workflow rationale) surfaced by atlas-lead before build delegations.
...
- Model tier table in `README.md`: lead on standard; premium reserved for gate roles via delegation.

## Actual usage in budget.md (v1)

The **Actual** block in `budget-template.md` is **self-reported** in v1. Roles and lead fill it at final delivery or retro from delegation notes and downgrade lines. There is no automatic token telemetry into `budget.md` yet.
...
| --- | --- | --- |
| Lead implements after quota hit | Collapses team model; burns lead context | Re-delegate at lower tier (`model-resilience.md`) |
| Skip kickoff budget | No envelope; heavy work starts unapproved | Phase 0 kickoff before requirements or build delegations |
...
| Mark delegation `completed` without success | False progress in `team.json` | Keep `active` or `failed` until subagent returns |
